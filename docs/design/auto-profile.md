...
- The extractor may not touch `[student]` lines. `apply_op` refuses a section rewrite that drops, changes, moves in or tags a line `[student]`, with an error phrased for the model's retry; the prompt says the same. Suppressions are now enforced in code too, not only asked for in the prompt: a rewrite that brings back a suppressed line is refused.
- They don't count toward the 5-line cap, which is about how much the extractor may assert. The 1500-char cap covers the whole document; an edit that would grow it past the cap is refused with the numbers, and one that shrinks it is always allowed.
- A save names the `rev` it started from. If a pass wrote in between, it is refused with a 409 and the client loads the latest version, because saving the stale text would suppress whatever the pass just added.
