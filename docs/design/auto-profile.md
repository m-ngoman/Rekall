...
- *Since 2026-09-25 a line can end `[student]` instead*: one the student wrote or reworded, with no evidence to show and nothing for staleness to act on. See §6.
...
*Revised 2026-09-25:* manual rows didn't stay untouched for long. Migration `7b0ab0075e93` folds each one into the profile as a `[student]` line, under the section its category maps to (context → Course and level, preference → What helps, gap and custom → How they work), then drops `student_memory_notes` and both its enums. The profile is now the only memory there is.
...
**Student edits.** *Revised 2026-09-25.* The first version moved an edited line into the student's own notes and suppressed the original, so the profile sat beside a separate list. The owner wanted one file, so the student now edits the document itself, and their lines live in it, tagged `[student]`:

- They edit the whole file as plain text: the three headings and the lines, without the evidence tags. On save, `student_profile.apply_edit` maps it back onto the stored document. A line whose text matches one of the tutor's keeps that line and its tag; any other line is the student's, filed under the heading it sits beneath (the first section if the heading is unknown). A tutor line that is gone is suppressed, exactly as a deletion is, and the next pass is a rebuild.
- So a reworded line is the old line suppressed plus a new `[student]` line. The next pass can't overwrite the student's wording or bring the old one back. That keeps the guarantee the notes design was chosen for, without the second list.
- The extractor may not touch `[student]` lines. `apply_op` refuses a section rewrite that drops, changes, moves in or tags a line `[student]`, or that writes one of the student's lines as its own; each error is phrased for the model's retry, and the prompt says the same. Suppressions are now enforced in code too, not only asked for in the prompt: a rewrite that brings back a suppressed line word for word is refused. A rewording can still only be caught by the prompt.
- Matching is by text, so text has to survive the editor's round trip. Lines are read with their spacing squeezed, and a line whose own text ends in something tag-shaped is refused on write: the editor shows lines without tags, and saving would read that tail off and turn the tutor's line into the student's. Where the tutor's line and the student's say the same thing, the student's is kept.
- They don't count toward the 5-line cap, which is about how much the extractor may assert. The 1500-char cap covers the whole document and refuses only growth, from either hand: notes folded in by the migration can leave a document over it, and the tutor must still be able to shrink its own lines, and the student theirs.
- A save names the `rev` it started from. If a pass wrote in between, it is refused with a 409, because saving the stale text would suppress whatever the pass just added. The client carries the edit over to the latest version — what the student took out comes out, what they added goes in under its heading — and puts it back in the editor to check and save. The panel also reloads the file each time it opens, so an edit rarely starts from an old version.
- A pass counts itself (`passes`) only against the revision it read, so a student's edit made during a pass keeps the rebuild it set up; and it makes the profile row only when it has something to write, so it never holds the row open across the model call.
...
**The student's lines, presented to the tutor.** *Revised 2026-09-25: one document now, not two blocks, with the asymmetry kept inside it.* `_memory_context` injects the whole profile under one header, which says that lines ending `[student]` the student wrote themselves, are reliable and may carry instructions, and that the other lines are the tutor's own impressions with the hedged framing and relevance gate above. The extractor still reads the student's lines as *constraints on what it may write* — Claude's `memory_user_edits` shape — so a line reading "stop telling me I rush" still suppresses that pattern, which is ChatGPT's "Don't mention this again" for free.
...
- ~~**A student-facing textarea over the document.**~~ *Reversed 2026-09-25.* The argument was that ChatGPT ships four narrow affordances instead, that the two labs disagree about presentation (OpenAI moved to one summary; Anthropic moved *away* from one summary to editable topics, for recoverability), and that the document is the right thing to inject while entries are the right thing to show. In practice the profile starts empty and stays hidden until a pattern recurs across two sessions, so what students saw was the list of notes, and the owner wanted one file. Entry-granularity survives on the server: `apply_edit` still works line by line, so a removed line is suppressed and a reworded one becomes the student's (§6).
