...
change constantly; check current numbers before trusting any of them.*
...
This is the finding I'd have most liked to know a year ago.

The tutor runs multi-turn conversations with a ~1,680-token system prompt. Obvious move: use the
cheap model. Haiku 4.5 costs roughly half what Sonnet 5 does per token, and the tutor didn't seem
to need a frontier model.

Measured over real voice sessions, Sonnet 5 came out at **~$0.045 per voice-hour against Haiku
4.5's ~$0.094**. The expensive model cost less than half as much.

The reason is prompt caching, and specifically this: **cache minimums are not monotonic across a
model family.** Haiku 4.5 needs a 4,096-token prefix before anything caches. Sonnet 5 needs 1,024.

My system prompt is ~1,680 tokens. On Sonnet it caches from the very first turn. On Haiku it sits
under the threshold and caches nothing — not until conversation history alone pushes the prefix
past 4,096 tokens, somewhere around turn 19. Most conversations never get there. A cached read
costs about a tenth of the input price, so Sonnet was paying a tenth on nearly every turn while
Haiku paid full freight on nearly all of them.

The lesson generalises past these two models: **per-token price is not cost.** If you have a
stable prompt prefix, the cache minimum is often the dominant term, and it doesn't move in the
direction you'd assume as you go down a model family. Check it against your actual prompt length
before picking on the price sheet.
...
grader for a while. It's genuinely good at what it's built for.

What it's built for turned out to be the wrong job.

**Prometheus is a rubric critic, not a teacher.** Asked why an answer was wrong, it produces
evaluator-speak — "lacks depth," "does not fully address the criteria." That's a useful signal if
you're scoring a benchmark. It is worthless to a student who wants to know what the right answer
actually was. My users don't need a verdict, they need the explanation attached to it.

It also wouldn't reliably follow output-format instructions, so its verdict needed a *second*
model to rewrite it into something presentable — two calls, two failure modes, more latency.

A general instruct model asked directly for both — grade this, and explain it the way a tutor
would — does the whole thing in one streamed call and follows the format. That's what ships.
...
## 3. I picked models on worst case, not average
...
**For anything inside an interactive loop, you're buying p99, not mean.** Three runs is a small
sample, but a 4x spread across three runs is enough signal to walk away.

Grading accuracy was equal across all three, incidentally. That's its own finding: for a
well-scaffolded grading prompt with a reference answer in context, the model tier stopped mattering
long before the price did.
...
I tried gemma-4-12B first. It took **4–5+ seconds to first token**. My initial assumption was
prompt length or a cold-load artifact, so I measured the prefill separately: **under 0.15s.** The
prompt wasn't the problem. A second back-to-back warm call was just as slow, so it wasn't loading
either. It was simply slow per-token generation on this hardware.

qwen2.5:7b on the identical prompt: **0.14s TTFT, ~1s total** once warm, with comparable
teaching-quality output.

The generalisable bit is the debugging move, not the model choice: **measure prefill and generation
separately before concluding anything about why a local model is slow.** "Big model is slow"
and "big prompt is slow" have completely different fixes, and I'd have optimised the wrong one for
a week.
...
The fix is a **holdback margin**: never flush the trailing 24 characters. Hold them until you
either see more content (proving they were real text) or the stream ends (at which point you parse
the marker out of the tail). 24 characters is just over the longest possible marker.

The general shape of this: **if a stream carries both content and control tokens, your flush
boundary has to lag the control token's maximum length.** Anything that mixes display text and
structured output in one stream has this problem, and it only shows up in front of real users,
because it's invisible when you're reading the completed response in a test.
...
Users have wildly different tolerances for being marked wrong. So strictness is a user setting —
but the naive implementation, where strictness only edits the prompt, produces incoherent
behaviour: the model *says* it's being lenient while the schedule punishes you exactly as hard,
because the score-to-grade mapping never changed.

Strictness now changes **both**: a prompt clause and a distinct score-to-grade mapping per level.
The wording and the consequence stay consistent.

Two other separations that turned out to matter:

**The explanation never touches the scheduler.** `grading_explanation` is a structurally separate
field from `grade`. Prose cannot leak into the algorithm, ever, no matter how the prompt drifts.

**"I don't know" isn't a wrong answer.** A blank or don't-know response routes to an explain-only
path that teaches the card rather than grading a non-attempt. Grading a non-attempt as a failure
is technically correct and pedagogically useless — and it made the app feel punitive in exactly
the moment a user was already discouraged.
...
**Budget provider evaluation in hours-to-working, not dollars-per-hour.** A 3x price difference on
a line item that rounds to nothing is not worth one afternoon, let alone three.
...
- **Price your prompt, not your tokens.** Cache minimums can invert the entire cost ordering of a
  model family.
- **Buy p99 for anything a user waits on.** Variance is the product problem; the mean is a
  benchmark artifact.
- **Purpose-built doesn't mean right.** A judge model judges. If you need it to teach, explain, or
  be read by a human, test the general model.
